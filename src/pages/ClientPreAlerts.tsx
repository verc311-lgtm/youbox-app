...
import { FileUp, Shield, MapPin, Clock, AlertCircle, ShieldCheck } from 'lucide-react';
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                prealertas.map((p) => {
                                    const hasPago = p.con_seguro && p.fondo_seguros && p.fondo_seguros.length > 0;
                                    return (
                                        <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="p-4">
                                                <span className="font-semibold text-slate-800 bg-slate-100 px-2 py-0.5 rounded text-sm font-mono">{p.tracking}</span>
                                            </td>
                                                ...
                                                {p.con_seguro ? (
                                                    hasPago ? (
                                                        <div className="flex flex-col gap-0.5">
                                                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 w-fit">
                                                                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                                                                Seguro Cubierto (${Number(p.monto_seguro).toFixed(2)})
                                                            </span>
                                                            <span className="text-[10px] text-emerald-600 font-medium">
                                                                Póliza activa y pagada
                                                            </span>
                                                        </div>
                                                    ) : (
                                                        <div className="flex flex-col gap-0.5">
                                                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200 w-fit">
                                                                <AlertCircle className="w-3.5 h-3.5 text-rose-600" />
                                                                Debe Seguro (${Number(p.monto_seguro).toFixed(2)})
                                                            </span>
                                                            <span className="text-[10px] text-rose-500 font-medium">
                                                                Pendiente de pago / comprobante
                                                            </span>
                                                        </div>
                                                    )
                                                ) : (
                                                    <span className="text-slate-500 text-xs font-medium">No (Renuncia)</span>
                                                )}
                                            </td>
