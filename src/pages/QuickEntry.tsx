...
export function QuickEntry() {
  ...
  const { data: transportistas = [], isLoading: isLoadingTransp } = useTransportistas();
  ...
  useEffect(() => {
    if (bodegas.length > 0) {
      ...
      if (rows.length === 0) {
        ...
        // If we already have rows but they have empty IDs (race condition fix)
        setRows(prev => prev.map(r => ({
          ...r,
          bodega_id: r.bodega_id || defBodega,
          transportista_id: r.transportista_id || defTrans
        })));
      }
    }
        ...
        <div className="overflow-x-visible custom-scrollbar">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100/50 bg-white/40">
              {(isLoadingBodegas || isLoadingTransp) && rows.length === 0 ? (
                <tr>
              ...
              {(!isLoadingBodegas && !isLoadingTransp) && rows.length === 0 && (
                <tr>
                  <td colSpan={9} className="py-20 text-center">
                    <p className="text-sm font-medium text-slate-400">No hay filas. Haz clic en "Añadir Fila".</p>
                  </td>
                </tr>
              )}
            </tbody>
